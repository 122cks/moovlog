...
export async function researchRestaurant(restaurantName) {
  const prompt = `구글 검색을 통해 '식당명: ${restaurantName}'에 대한 최신 블로그 리뷰와 정보를 조사하고, 아래 항목을 200자 이내로 요약하세요:
• 시그니처 메뉴와 맛의 특징 (예: 육즘 가득한 수제버거, 30년 전통 간장게장 등)
• 이 식당이 특별한 이유 (예: 사장님이 직접 수확한 재료, 독자 개발 소스 등)
• 분위기 및 방문 상황 (예: 데이트, 친구 모임, 가족 나들이)
• 가격대 정보
없는 정보는 생략하고 발견한 정보만 간결하게 요약하세요.`;
...
${trendInstruction}[사용자 특별 요청 사항 (★이 지시사항을 최우선으로 반영할 것★)]
...
[자막 규칙 — 세련되고 직관적인 카피]
caption1: 억지스러운 감탄사 금지. 해당 컷의 시각적 포인트나 맛의 핵심을 찌르는 4~10자 (예: "입안 가득 차는 육즙", "웨이팅이 아깝지 않은 이유")
caption2: narration의 감상을 압축한 4~10자 (없으면 빈 문자열)
subtitle_style: hook(첫 씬) | hero(음식 클라이맥스) | cta(마지막 행동유도) | minimal(여운/감성) | elegant(에세이 스타일)
...
{"title":"제목","hashtags":"#태그","naver_clip_tags":"#협찬 #서울맛집","youtube_shorts_tags":"#맛집 #shorts","instagram_caption":"소개문\\n\\n#태그","tiktok_tags":"#맛집","scenes":[
  {"idx":0,"media_idx":2,"duration":3.0,"caption1":"입안 가득 차는 육향","caption2":"숯불향이 은은해요","subtitle_style":"hook","subtitle_position":"center","narration":"두툼하게 썰어낸 채끝, 숯불 위에서 은은하게 피어오르는 향이 기대감을 높여요.","effect":"zoom-out"}
]}`;
