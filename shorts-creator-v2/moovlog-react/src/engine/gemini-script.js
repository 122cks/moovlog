...
` : '';
  ...
  // ─── 고깃집 전용: 업로드된 미디어의 food_category 기반으로 사용 가능한 씬 결정 ───
  ...
  // 첫 블록: 식전주스 OR 상차림 미디어가 있을 때만 포함
  const grillBlock1 = (grillHasJuice || grillHasTable)
    ...
    : `블록1 훅(total_duration 3.0~4.5s): 이 식당만의 가장 강렬한 첫 비주얼로 시청자를 멈추게 하는 오프닝. retention_strategy: "opening_question", energy_level 4~5.`;
  const grillBlock2 = grillHasSideDish
    ? `블록2 밑반찬(total_duration 2.5~3.5s): 정갈한 밑반찬 클로즈업 또는 풀샷. energy_level 2~3.`
    : '';
  const grillBlock3 = grillHasSoup
    ? `블록3 찌개·계란찜 등 보조 요리(total_duration 3.0~4.0s): 찌개·계란찜·된장국 등 서브 요리. retention_strategy: "midpoint_reveal", energy_level 3.`
    : '';
  const grillBlock6 = grillHasFriedRice
    ? `블록6 마무리 볶음밥/냉면(total_duration 2.5~3.5s): 마무리 메뉴 클로즈업. energy_level 2~3.`
    : '';
  const grillBlocks = [
    grillBlock1,
    grillBlock2,
