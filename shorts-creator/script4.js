...
const AUTO_EXPORT_ON_CREATE = false; // 모바일 UX: 사용자가 직접 저장 버튼 누르도록
...
const D  = {
  ...
  vProg: g('vProg'), vProgText: g('vProgText'),
  playBtn: g('playBtn'), playIco: g('playIco'),
  ...
  resultSub: g('resultSub'),
  audioBadge: g('audioBadge'), audioBadgeText: g('audioBadgeText'),
  bgmBadge: g('bgmBadge'), bgmBadgeText: g('bgmBadgeText'),
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (D.apkBannerSub) D.apkBannerSub.textContent = `moovlog-shorts-creator-${APP_VERSION}.apk →`;
...
function renderTemplatePicker() {
  ...
  if (!picker) return;
  // "🤖 AI 자동" 칩 포함 (기본 선택)
  const autoChip = `<button class="tpl-chip tpl-chip-auto${selectedTemplate === 'auto' ? ' active' : ''}" data-tpl="auto">🤖 AI 자동</button>`;
  picker.innerHTML = autoChip + Object.entries(TEMPLATE_NAMES).map(([id, name]) =>
    `<button class="tpl-chip${id === selectedTemplate ? ' active' : ''}" data-tpl="${id}">${name}</button>`
  ).join('');
  picker.querySelectorAll('.tpl-chip').forEach(btn => {
    btn.addEventListener('click', () => {
      selectedTemplate = btn.dataset.tpl === 'auto' ? 'aesthetic' : btn.dataset.tpl;
      picker.querySelectorAll('.tpl-chip').forEach(b => b.classList.remove('active'));
      btn.classList.add('active');
      // hidden input 동기화
      if (D.selectedTplInput) D.selectedTplInput.value = btn.dataset.tpl;
      // 영상 생성 완료 상태면 즉시 재렌더 (프리뷰)
      if (S.script && S.loaded.length) renderFrame(S.scene, S.subAnimProg);
      toast(`스타일: ${btn.dataset.tpl === 'auto' ? '🤖 AI 자동' : TEMPLATE_NAMES[selectedTemplate]}`, 'inf');
    });
  });
}
...
function setupPlayer() {
  ...
  renderFrame(0, 0); setPlayIcon(false);
}
...
function doReplay() {
  pausePlay(); S.scene = 0; S.startTs = null; S.subAnimProg = 0;
  D.vProg.style.width = '0%';
  if (D.vProgText) D.vProgText.textContent = '0%';
  renderFrame(0, 0); highlightScene(0); setTimeout(startPlay, 80);
}
function toggleMute() {
  S.muted = !S.muted;
...
function tick() {
  const run = now => {
    ...
    const pct = Math.round((done + el) / total * 100);
    D.vProg.style.width = pct + '%';
    if (D.vProgText) D.vProgText.textContent = pct + '%';
    ...
    if (prog >= 1) {
      if (S.scene < S.script.scenes.length - 1) {
        ...
        D.vProg.style.width = '100%';
        if (D.vProgText) D.vProgText.textContent = '100%';
        S.playing = false; stopAudio(); setPlayIcon(false); return;
      }
    }
