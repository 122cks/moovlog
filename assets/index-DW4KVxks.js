import { w as create, x as devtools, y as initializeApp, z as getStorage, A as getFirestore, B as ref, D as uploadBytes, G as getDownloadURL, H as addDoc, I as collection, J as serverTimestamp, K as updateDoc, M as doc, N as query, O as orderBy, P as limit, Q as getDocs, R as setDoc, T as onSnapshot, U as where, V as getDoc, W as deleteDoc, X as FFmpeg, Y as fetchFile, Z as jsxRuntimeExports, $ as reactExports, a0 as reactDomExports, a1 as Muxer, a2 as Muxer$1, a3 as Mp4Muxer, a4 as WebmMuxer, a5 as client, a6 as React } from './vendor-DRc8VREz.js';
import { g as getAuth, o as onAuthStateChanged, s as signInAnonymously } from './vendor-firebase-D67b0poq.js';
...
async function firebaseUploadOriginals(files, restaurantName, pipelineSessionId) {
  ...
  await Promise.all(
    files.map(
      (m, i) => fbUpload(m.file, `originals/${session}/${i}_${m.file.name}`).catch((e) => console.warn(`[Firebase] ?뚯씪 ${i} ?낅줈???ㅽ뙣:`, e.message))
    )
  );
}
...
async function saveEDLToFirestore(edl, projectId = null) {
  if (!db) return null;
  try {
    const data = {
      ...edl,
      ...
      platform: typeof window !== "undefined" && window.electronAPI?.isElectron ? "electron" : "web"
    };
    if (projectId) {
...
function subscribeEDL(projectId, onChange) {
...
async function loadProjects(maxCount = 20) {
  if (!db) return [];
  try {
...
async function exportSceneData(projectId, sceneData) {
  if (!db) return;
  try {
...
async function importSceneData(projectId) {
  if (!db) return null;
  try {
...
async function mergeWithConflictResolution(projectId, localEDL) {
...
async function fetchWithTimeout$1(url, options, timeout = 6e4) {
  ...
  try {
    ...
    if (e.name === "AbortError") throw new Error(`요청 시간 초과(${Math.round(timeout / 1e3)}s 제한)`);
    throw e;
  } finally {
    clearTimeout(id);
  }
 ...
 */
async function warmupFFmpeg(onProgress) {
  if (ffmpegInstance) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("header", { className: "app-header", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "header-inner", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "header-version", children: "v2.73" })
      ] }),
...
[🎬 레퍼런스 타겟 — 인스타그램 릴스·유튜브 쇼츠 가게 홍보 영상 공식]
이 영상은 한국 인스타·쇼츠에서 실제로 바이럴된 맛집 가게 홍보 숏폼 영상의 공식을 그대로 구현합니다.
성공한 포맷(뭐먹지·망고플레이트·힛더로드·성수동 F&B 릴스 인플루언서)의 공통 타임라인:
▶ 0.0~2.0s  │ 함정 오프닝(HOOK): 가장 강렬한 음식 비주얼 먼저 — 먹방 직전·육즙 흐름·비주얼 충격
▶ 2.0~10.0s │ 스토리텔링: 상차림→재료/텍스처→조리 과정을 교차편집, "왜 이 집이냐" 팩트 전달
▶ 10.0~16.0s│ 클라이맥스: 이 집의 시그니처 비주얼 최절정 (육즙·완성된 요리·분위기)
▶ 16.0~20.0s│ 아웃트로: 매장 분위기/외관 + CTA ("저장하고 꼭 가보세요")

[🤖 캡컷(CapCut) AI 자동컷 기술 — 반드시 준수]
실제 캡컷 AI 자동 생성이 적용하는 편집 알고리즘을 구현합니다:
• BPM 교차 편집: 짧은컷(0.5~1.0s) ↔ 긴컷(1.5~2.5s) 리드미컬하게 교차. 같은 길이 3컷 이상 연속 금지.
• 시점 순환 룰: 매 3~4컷마다 앵글 변환 (클로즈업 → 풀샷 → 측면/사이드뷰 → 클로즈업 반복).
• 클라이맥스 직전 패턴: 0.5~0.7s 초SHORT 컷으로 긴장감 고조 → 2.0~2.5s 안정컷으로 해소 (캡컷 J-Cut).
• 동일 media_idx 2연속 배치 절대 금지 (연속 반복 = 지루함의 신호).
• 영상(video) media_idx를 클라이맥스(가장 역동적인) 컷에 우선 배치.
• 나레이션이 없는 첫 0.5~1.0s 컷으로 시작 → 시각 함정 먼저, 목소리는 뒤따라옴.
...
★ 캡컷 AI 자동컷 원칙: 인접한 두 컷은 반드시 다른 effect를 사용하세요 (zoom-in→zoom-in 연속 금지). ★
...
function saveDriveSession(files) {
  try {
    const payload = {
...
// ─── 완료된 생성 결과 저장 / 복원 ───────────────────────────
function saveLastResult(script, restaurantName, qcScore) {
  try {
 ...
 * }>>}
 */
async function detectSceneCuts(videoFile, {
 ...
 */
async function detectBeatTimings(audioFile) {
  try {
 ...
 * }>}
 */
function buildBeatAlignedEDL(sceneCuts, beatTimings, targetDuration = 30) {
 ...
 */
async function enrichAnalysisWithSceneData(files, analysis, onProgress) {
...
function UploadSection() {
  ...
  const saveDraftTimer = reactExports.useRef(null);
  ...
  const [driveResume, setDriveResume] = reactExports.useState(null);
  const [prevResult, setPrevResult] = reactExports.useState(null);
  ...
  const handleReDriveDownload = reactExports.useCallback(async (driveFiles) => {
  ...
  const handleRestoreResult = reactExports.useCallback((saved) => {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("main", { className: "app-main", children: [
    (driveResume || prevResult) && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "card", style: { marginBottom: 14, border: "1px solid rgba(124,58,237,0.4)", background: "rgba(124,58,237,0.07)", padding: "12px 16px" }, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { style: { margin: "0 0 10px", fontWeight: 700, fontSize: "0.88rem", color: "#c4b5fd" }, children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("i", { className: "fas fa-redo-alt", style: { marginRight: 6 } }),
        "이전 작업 이어서 진행"
      ] }),
      driveResume && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", alignItems: "center", gap: 10, padding: "6px 0" }, children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("svg", { width: "14", height: "12", viewBox: "0 0 87.3 78", xmlns: "http://www.w3.org/2000/svg", style: { flexShrink: 0 }, children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("path", { d: "m6.6 66.85 3.85 6.65c.8 1.4 1.95 2.5 3.3 3.3l13.75-23.8h-27.5c0 1.55.4 3.1 1.2 4.5z", fill: "#0066da" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("path", { d: "m43.65 25-13.75-23.8c-1.35.8-2.5 1.9-3.3 3.3l-25.4 44a9.06 9.06 0 0 0 -1.2 4.5h27.5z", fill: "#00ac47" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("path", { d: "m73.55 76.8c1.35-.8 2.5-1.9 3.3-3.3l1.6-2.75 7.65-13.25c.8-1.4 1.2-2.95 1.2-4.5h-27.502l5.852 11.5z", fill: "#ea4335" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("path", { d: "m43.65 25 13.75-23.8c-1.35-.8-2.9-1.2-4.5-1.2h-18.5c-1.6 0-3.15.45-4.5 1.2z", fill: "#00832d" }),
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { style: { flex: 1, fontSize: "0.81rem", color: "#ccc" }, children: [
          "Drive ",
          driveResume.files.length,
          "개 파일 — ",
          driveResume.files.slice(0, 2).map((f) => f.name).join(", "),
          driveResume.files.length > 2 ? ` 외 ${driveResume.files.length - 2}개` : ""
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
      ...
      prevResult && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", alignItems: "center", gap: 10, padding: "6px 0", borderTop: driveResume ? "1px solid rgba(255,255,255,0.07)" : "none", marginTop: driveResume ? 8 : 0 }, children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("i", { className: "fas fa-film", style: { color: "#a78bfa", fontSize: "1rem", flexShrink: 0 } }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { style: { flex: 1, fontSize: "0.81rem", color: "#ccc" }, children: [
          prevResult.restaurantName,
          " — 이전 생성 결과",
...
function SceneEditor({ sceneIdx, onClose }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "modal-backdrop", onClick: onClose, children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "modal-box", onClick: (e) => e.stopPropagation(), children: [
    ...
    files?.length > 1 && /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "modal-label", children: "미디어 교체 (A-B Roll)" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: {
        display: "grid",
        ...
        return /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "div",
          {
            ...
            children: [
              f.type === "video" ? /* @__PURE__ */ jsxRuntimeExports.jsx("video", { src: f.url, muted: true, playsInline: true, style: { width: "100%", height: "100%", objectFit: "cover", pointerEvents: "none" } }) : /* @__PURE__ */ jsxRuntimeExports.jsx("img", { src: f.url, alt: `미디어 ${i + 1}`, style: { width: "100%", height: "100%", objectFit: "cover" } }),
...
function RenderProgressBar({ jobId, onClose }) {
  ...
  const startTs = reactExports.useRef(Date.now());
  ...
  reactExports.useEffect(() => {
    ...
    timerRef.current = setInterval(() => {
      setElapsed(Math.floor((Date.now() - startTs.current) / 1e3));
    }, 1e3);
    const unsubProgress = api.onRenderProgress(({ pct: p, msg, jobId: jid }) => {
      ...
      if (msg) setLogs((prev) => [...prev.slice(-199), `[${(/* @__PURE__ */ new Date()).toLocaleTimeString()}] ${msg}`]);
      if (p >= 100) {
        setStatus("done");
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: {
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { style: { fontWeight: 700, fontSize: 14 }, children: [
        "🎬 렌더링 ",
        /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { style: { color: statusColor, fontSize: 12 }, children: [
          "● ",
          statusLabel
        ] })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { style: { fontSize: 12, color: "#aaa" }, children: [
        "경과: ",
        fmtTime(elapsed)
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { background: "#0f0f2a", borderRadius: 6, height: 16, marginBottom: 8, overflow: "hidden" }, children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: {
      height: "100%",
      width: `${pct}%`,
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 12 }, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
        pct,
        "%"
      ] }),
      status === "done" && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { color: "#60a5fa" }, children: "✓ 렌더링 완료" })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", gap: 8, marginBottom: 12 }, children: [
      status === "running" && /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: handlePause, style: btnStyle("#fbbf24"), children: "⏸ 일시정지" }),
      status === "paused" && /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: handleResume, style: btnStyle("#4ade80"), children: "▶ 재개" }),
      (status === "running" || status === "paused") && /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: handleCancel, style: btnStyle("#f87171"), children: "✕ 취소" }),
      (status === "done" || status === "error") && /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: onClose, style: btnStyle("#60a5fa"), children: "닫기" })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      "div",
      {
        ...
        children: logs.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { color: "#555" }, children: "FFmpeg 로그가 여기 표시됩니다..." }) : logs.map((l, i) => /* @__PURE__ */ jsxRuntimeExports.jsx("div", { children: l }, i))
      }
    )
  ] });
}
...
function ExportSettings({ onSelect, selected = "balanced", disabled = false }) {
  const [presets, setPresets] = reactExports.useState(WEB_PRESETS);
  const [hwInfo, setHwInfo] = reactExports.useState(null);
  const [loading, setLoading] = reactExports.useState(false);
  const api = typeof window !== "undefined" ? window.electronAPI : null;
  reactExports.useEffect(() => {
    if (!api) return;
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: styles.container, children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: styles.header, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: styles.title, children: "⚙️ 내보내기 설정" }),
      loading && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: styles.loadingBadge, children: "감지 중..." })
    ] }),
    hwInfo && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: styles.hwInfo, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { style: { color: hwInfo.hwaccel !== "libx264" ? "#4ade80" : "#888" }, children: [
        hwInfo.hwaccel !== "libx264" ? "⚡" : "🔧",
        " ",
        hwInfo.hwaccel || "libx264"
      ] }),
      hwInfo.cpuThreads && /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { style: styles.hwChip, children: [
        hwInfo.cpuThreads,
        " 스레드"
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: styles.hwChip, children: hwInfo.version?.split(" ")[2] || "FFmpeg" })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: styles.presetList, children: presets.map((p) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "button",
      {
        disabled,
        onClick: () => onSelect(p),
        ...
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: styles.presetTop, children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: styles.presetLabel, children: p.label }),
            p.recommended && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: styles.badge, children: "추천" }),
            p.twoPass && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { ...styles.badge, background: "#7c3aed" }, children: "2-pass" })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: styles.presetDesc, children: p.desc })
        ]
      },
      p.id
    )) }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: styles.summary, children: [
      "선택: ",
      /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: current.label }),
      current.crf && /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
        " · CRF ",
        current.crf
      ] }),
      current.res && /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
        " · ",
        current.res
      ] }),
      current.twoPass && /* @__PURE__ */ jsxRuntimeExports.jsx(jsxRuntimeExports.Fragment, { children: " · 2-pass 인코딩" })
    ] })
  ] });
}
const styles = {
 ...
 */
async function shareVideo(blob, text, filename = 'moovlog.mp4') {
 ...
 */
async function extractVideoThumbnail(videoEl, timeSeconds = 1, opts = {}) {
...
// #84  썸네일 다운로드
// ═══════════════════════════════════════════════════════════════════════════
function downloadThumbnail(blob, restaurantName = 'moovlog') {
  const url = URL.createObjectURL(blob);
...
// ═══════════════════════════════════════════════════════════════════════════
function openInstagramShare(caption) {
...
// #86  TikTok 딥링크
// ═══════════════════════════════════════════════════════════════════════════
function openTikTokShare(caption) {
  copyToClipboard(caption);
  const isMobile = /iPhone|iPad|Android/i.test(navigator.userAgent);
  if (isMobile) {
 ...
 */
function getBrandingWatermark(restaurantName, handle = '@moovlog') {
  return {
...
async function autoMatchScenes(blogText, sceneCount = 6) {
  const prompt = `
...
async function generateSubtitle(audioBuffer, startTime, hintText = "") {
...
function detectBeatDrops(buffer, threshold = 0.7) {
  if (!buffer) return [];
...
async function suggestLUT(text) {
...
async function suggestHashtags(restaurantName, script = "", platform = "all") {
  const prompt = `
당신은 맛집 SNS 마케터입니다.
식당명: ${restaurantName}
...
function autoSplitToShorts(scenes, targetDuration = 30) {
  const clips = [];
  let current = [];
  let total = 0;
...
function getKenBurnsFilter(sceneIndex, duration) {
  const effects = [
    // 줌인 (중앙→확대)
...
function ExportPanel() {
  const {
    ...
    setExporting,
    exporting,
    pipelineSessionId: pipelineSessionId2,
    files,
    ffmpegBlob,
    ffmpegRendering,
    ffmpegProgress,
  ...
  const [selectedPreset, setSelectedPreset] = reactExports.useState("balanced");
  const [presetObj, setPresetObj] = reactExports.useState(null);
  const [electronJobId, setElectronJobId] = reactExports.useState(null);
  const [showSettings, setShowSettings] = reactExports.useState(false);
  const [hashtags, setHashtags] = reactExports.useState(null);
  const [hashtagBusy, setHashtagBusy] = reactExports.useState(false);
  const lastBlobRef = reactExports.useRef(null);
  ...
  const doElectronRender = async () => {
    const api = window.electronAPI;
    if (!api?.renderVideo) {
      addToast("Electron 앱에서만 사용 가능합니다", "inf");
      return;
    }
    if (!script?.scenes?.length) {
      addToast("먼저 영상을 생성해주세요", "err");
...
${tags}`, `moovlog_${sanitizeName(restaurantName)}.mp4`);
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dl-box", children: [
    ...
    ffmpegRendering && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { marginTop: "8px" }, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", alignItems: "center", gap: "6px", fontSize: "0.75rem", color: "#a855f7" }, children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("i", { className: "fas fa-spinner fa-spin" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: ffmpegMsg || "시네마틱 렌더링 중..." }),
        ffmpegProgress > 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { style: { marginLeft: "auto" }, children: [
          ffmpegProgress,
          "%"
        ] })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { marginTop: "4px", background: "rgba(255,255,255,0.08)", borderRadius: "6px", overflow: "hidden", height: "6px" }, children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: {
        height: "100%",
        background: "linear-gradient(90deg,#7c3aed,#a855f7)",
        width: `${ffmpegProgress}%`,
        transition: "width 0.4s ease",
        borderRadius: "6px"
      } }) })
    ] }),
    ffmpegBlob && !ffmpegRendering && /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "button",
      {
        className: "dl-audio-btn",
        onClick: doCinematicDownload,
        ...
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("i", { className: "fas fa-film" }),
          " 🎬 시네마틱 MP4 저장"
        ]
      }
    ),
    ...
    typeof window !== "undefined" && window.electronAPI?.isElectron && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { marginTop: 12, borderTop: "1px solid rgba(255,255,255,0.08)", paddingTop: 12 }, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }, children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { fontSize: 13, fontWeight: 700, color: "#a5b4fc" }, children: "🖥️ 로컬 고품질 렌더" }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "button",
          {
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { marginTop: 12, borderTop: "1px solid rgba(255,255,255,0.08)", paddingTop: 12 }, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { fontSize: 13, fontWeight: 700, color: "#a5b4fc", marginBottom: 8 }, children: "📲 공유 & 해시태그" }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", gap: 6, flexWrap: "wrap" }, children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "button",
          {
            ...
            children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("i", { className: "fas fa-share-nodes" }),
              " 영상 공유"
            ]
          }
        ),
        /* @__PURE__ */ jsxRuntimeExports.jsxs(
          ...
          {
            className: "dl-audio-btn",
            onClick: doGenHashtags,
            disabled: hashtagBusy,
            style: { flex: 1, minWidth: 120 },
            title: "AI 해시태그 자동 생성",
            children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("i", { className: `fas ${hashtagBusy ? "fa-spinner fa-spin" : "fas fa-hashtag"}` }),
              hashtagBusy ? " 생성 중..." : " AI 해시태그"
            ]
          }
        )
      ] }),
      hashtags && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { marginTop: 8, fontSize: 11, color: "#888", display: "flex", flexDirection: "column", gap: 4 }, children: [["instagram", "🟣 IG"], ["tiktok", "⚫ TT"], ["naver", "🟢 블로그"]].map(([key, label]) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", alignItems: "flex-start", gap: 6 }, children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "button",
          {
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { style: { flex: 1, wordBreak: "break-word", color: "#666" }, children: [
          hashtags[key]?.slice(0, 60),
          "..."
        ] })
      ] }, key)) })
    ] })
...
function LoadedKitTabs({ kit, addToast }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
    kit.hookTitle && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "marketing-row", style: { marginTop: 12, marginBottom: 8 }, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }, children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "marketing-label", style: { margin: 0 }, children: "🎣 훅 제목" }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: "marketing-copy-btn", onClick: () => copy(kit.hookTitle), children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("i", { className: "fas fa-copy" }),
          " 복사"
        ] })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "marketing-text", style: { margin: 0, whiteSpace: "pre-line", fontSize: "0.82rem" }, children: kit.hookTitle })
    ] }),
    TABS.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { display: "flex", gap: 6, flexWrap: "wrap", marginTop: 10 }, children: TABS.map((tab) => /* @__PURE__ */ jsxRuntimeExports.jsx(
      "button",
      {
    ...
    kit.hookVariations?.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "marketing-row", style: { marginTop: 12, marginBottom: 0 }, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "marketing-label", children: "🎣 3종 훅 베리에이션" }),
      kit.hookVariations.map((h, i) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { background: "rgba(0,0,0,0.3)", borderRadius: 8, padding: "8px 12px", marginTop: 6, fontSize: "0.75rem" }, children: [
    ...
    openTab && /* @__PURE__ */ jsxRuntimeExports.jsx(
      "div",
      {
        style: {
          position: "fixed",
        ...
        children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "div",
          {
            style: {
              background: "#1a1a1a",
              border: `1.5px solid ${openTab.color}55`,
              borderRadius: 18,
              padding: "24px 22px",
              maxWidth: 480,
              width: "92%",
            ...
            children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }, children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { color: openTab.color, fontWeight: 800, fontSize: "1rem" }, children: openTab.label }),
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "button",
                  {
                    onClick: () => setOpenTab(null),
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { style: {
                ...
                borderRadius: 10,
                padding: "12px 14px"
              }, children: openTab.val }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 14 }, children: [
                /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { style: { fontSize: "0.68rem", color: "#555" }, children: [
                  openTab.val.length,
                  "자"
                ] }),
                /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: "marketing-copy-btn", style: { float: "none" }, onClick: () => copy(openTab.val), children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("i", { className: "fas fa-copy" }),
                  " 복사"
                ] })
              ] })
...
function ResultScreen() {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "result-wrap", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "result-inner", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { ref: kitPanelRef, className: "marketing-assets-box", style: { marginTop: 8, ...loadedKit ? { border: "1.5px solid #7c3aed66", background: "rgba(124,58,237,0.07)" } : {} }, children: [
      ...
      loadedKit && /* @__PURE__ */ jsxRuntimeExports.jsx(LoadedKitTabs, { kit: loadedKit, addToast })
    ] }),
...
function App() {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "app-root", children: [
    ...
    ffmpegWarmMsg && !ffmpegReady && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: {
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("i", { className: "fas fa-spinner fa-spin", style: { flexShrink: 0 } }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: ffmpegWarmMsg }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { marginLeft: "auto", opacity: 0.6, fontSize: "0.68rem" }, children: "(두 번째 접속부터 즉시 시작)" })
    ] }),
    ffmpegReady && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: {
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "🎬 시네마틱 렌더링 엔진 준비 완료" })
    ] }),
